        ...
        <div class="flex flex-wrap items-center">
            ...
            <div class="flex flex-1 md:w-1/3 justify-center md:justify-start text-white px-2">
                <span class="relative w-full">
                    <input aria-label="search" type="search" id="search" placeholder="Search" class="w-full bg-gray-900 text-white transition border border-transparent focus:outline-none focus:border-gray-400 rounded py-3 px-7 pl-10 appearance-none leading-normal">
                ...
                <div class="flex flex-wrap">
                    ...
                    <div class="w-full p-6">
                        ...
                        if(isset($_GET['id']))
                            ...
                            $evenementId = mysqli_real_escape_string($db, $_GET['id']);
                            $query = "SELECT * FROM evenementen WHERE id='$evenementId' ";
                            $query_run = mysqli_query($db, $query);

                            if(mysqli_num_rows($query_run) > 0)
                       
                                $evenement=mysqli_fetch_array($query_run);
                                ?>
                              <form action="script.php" method="POST">
                              <input type="hidden" name="evenementId" value="<?= $evenement['id']; ?>">
                      <div class="mb-3">
                      <img class="image is-128x128" src="../images/<?= $evenement['image'] ?>"/>
                      <p>Datum: <?= $evenement['evname'] ?></p>
    <p>Datum: <?= $evenement['date'] ?></p>
    <p>Descriptie: <?= $evenement['description'] ?></p>
                      <div class="mb-3">
                      <button class="button is-danger mt-4" type='submit' name='delete'>Delete</button>
